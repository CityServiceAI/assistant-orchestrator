...
U.4.3: Засмічення/проблема з ливневою каналізацією (на дорозі/вулиці) (MunicipalUtility_Water) | Централізоване водовідведення (ливневка)
...
2.  **Success (З достатньою інформацією):** Якщо інформації достатньо для визначення найбільш вірогідного коду з класифікатора (навіть якщо це не ідеальна відповідність), встановіть `need_clarification: false`. Заповніть усі поля в блоці "problem" та призначте відповідний confidence (наприклад, 0.7-0.8). **Не зациклюйтесь на нескінченних уточненнях, намагайтеся класифікувати проблему, якщо це можливо.**
3.  **Clarification (Лише якщо інформації критично недостатньо):** Якщо класифікація неможлива (наприклад, "щось тече, не знаю де") або інформації явно недостатньо для вибору коду, встановіть `need_clarification: true` і згенеруйте уточнення. Уточнююче запитання має активно пропонувати користувачеві конкретні варіанти відповідей.
