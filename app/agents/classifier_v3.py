...
from app.data.vector_db import setup_vector_database_from_csv
...
def search_problem_category_by_tags(tags_list, n_results=3):
    """
...
COLLECTION = setup_vector_database_from_csv(CATEGORIES_2_CSV)
...
class ClassifierV3:
    ...
    def run(self, state):
        ...
        rag_results = search_problem_category_by_tags(step1_data.get("tags", []), 5)
        logging.info(f"Rag results: {json.dumps(rag_results, indent=2, ensure_ascii=False)}")


        rag_results = search_problem_category_by_tags(step1_data.get("tags", []), 10)
        logging.info(f"Rag results: {json.dumps(rag_results, indent=2, ensure_ascii=False)}")

        formatted_prompt = AGENT_CONFIG['prompts']['system_prompt_step2'].format(
            current_datetime=get_current_date_info(),
            user_complaint=user_message,
            potential_categories_json=json.dumps(rag_results, indent=2, ensure_ascii=False),
